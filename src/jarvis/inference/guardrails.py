...
class Guardrails:
    ...
    def _strip_anonymisation_placeholders(self, text: str) -> str:
        ...
        # Το άρθρο που κυβερνούσε το placeholder μένει ορφανό.
        #
        # Παρατηρήθηκε ζωντανά: το μοντέλο αντέγραψε από το αρχείο τη φράση
        # «θα προσπαθησω να θυμηθω να παρω και την [NAME] το απογευμα», το
        # placeholder αφαιρέθηκε εδώ, και ο χρήστης είδε «…να πάρω και ΤΗΝ
        # το απόγευμα».
        #
        # Το ``_close_gap`` υπήρχε αλλά έτρεχε μόνο μέσα στο
        # ``_strip_surnames``, και μόνο όταν αυτό είχε όντως αφαιρέσει
        # επώνυμο. Η αφαίρεση placeholder — η συχνότερη αιτία χάσματος,
        # αφού το corpus είναι γεμάτο [NAME] — δεν το καλούσε ποτέ.
    ...
    #: Άρθρο ορφανό στη ΜΕΣΗ πρότασης.
    #:
    #: Το μοτίβο από πάνω απαιτεί στίξη ή τέλος γραμμής μετά το άρθρο, και
    #: πιάνει μόνο «…μίλησα με την.» Οι πραγματικές περιπτώσεις είναι στη
    #: μέση:
    #:
    #:     «να πάρω και ΤΗΝ ΤΟ απόγευμα»       (άρθρο + άρθρο)
    #:     «θα περάσω από ΤΗΝ ΓΙΑ να δω»       (άρθρο + πρόθεση)
    #:     «και Ο ΕΙΝΑΙ πολύ καλός καθηγητής»  (άρθρο + ρήμα)
    #:
    #: Το τρίτο παράδειγμα το αναφέρει το κεφάλαιο 7 ως **διορθωμένο**. Δεν
    #: ήταν: το test που το κατοχύρωνε έλεγχε συμβολοσειρά που τελείωνε στο
    #: άρθρο, δηλαδή τη μία μορφή που ήδη δούλευε.
    #:
    #: Ανιχνεύονται μόνο ασφαλείς ακολουθίες — λέξεις που ΔΕΝ μπορούν να
    #: είναι το ουσιαστικό του άρθρου. Ένα γενικό «άρθρο + οτιδήποτε» θα
    #: έσβηνε σωστά ελληνικά.
    _ORPHAN_ARTICLE_MIDSENTENCE = re.compile(
        r"\b(ο|η|το|τον|την|του|της|τους|τις|οι|τα)\s+"
        r"(?=(?:"
        r"ο|η|το|τον|την|του|της|τους|τις|οι|τα"          # άλλο άρθρο
        r"|για|από|απο|με|σε|προς|κατά|κατα|χωρίς|χωρις"  # πρόθεση
        r"|είναι|ειναι|ήταν|ηταν|έχει|εχει|θα|να|και"     # ρήμα/σύνδεσμος
        r")\b)",
        re.IGNORECASE,
    )

    #: Πρόθεση + άρθρο + επίρρημα: «με τον [NAME] μετά τη δουλειά».
    #:
    #: Στενότερος από τον γενικό κανόνα επίτηδες. Μια πρώτη εκδοχή έβαζε τα
    #: επιρρήματα στη γενική λίστα και κατέστρεψε το «το μετά το βλέπουμε»,
    #: όπου το «μετά» είναι ουσιαστικοποιημένο και το άρθρο σωστό. Εδώ
    #: απαιτείται πρόθεση μπροστά, που κάνει την ακολουθία αδιαμφισβήτητα
    #: σπασμένη: «με τον μετά» δεν είναι ελληνικά με κανέναν τρόπο.
    _ORPHAN_AFTER_PREPOSITION = re.compile(
        r"\b(?:με|από|απο|για|σε|προς|μαζί|μαζι)\s+"
        r"(?:τον|την|το|του|της|τους|τις|τα|οι)\s+"
        r"(?=(?:μετά|μετα|πριν|χθες|χτες|σήμερα|σημερα|αύριο|αυριο|"
        r"τώρα|τωρα|αργότερα|αργοτερα)\b)",
        re.IGNORECASE,
    )
    ...
    def _close_gap(self, text: str) -> str:
        ...
        while cleaned != previous:
            ...
            cleaned = self._ORPHAN_ARTICLE_MIDSENTENCE.sub("", cleaned)
            cleaned = self._ORPHAN_AFTER_PREPOSITION.sub("", cleaned)
            cleaned = re.sub(r"\s+([,.;!?·])", r"\1", cleaned)
            cleaned = re.sub(r"\s{2,}", " ", cleaned)
