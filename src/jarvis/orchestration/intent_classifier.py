...
_PERSONAL_PATTERNS: list[tuple[str, float]] = [
    ...
    # «ποιο μοντέλο χρησιμοποίησες;» — από τις πρώτες ερωτήσεις κάθε
    # εξέτασης, και δεν ταίριαζε πουθενά. Έπιανε μόνο αν ο εξεταστής έλεγε
    # το όνομα του μοντέλου, δηλαδή αν ήξερε ήδη την απάντηση.
    (r"\b(ποιο|ποια|τι|τί)\s+(μοντ[εέ]λο|dataset|corpus|δεδομ[εέ]να)\b", 0.85),
    (r"\bπ[ωώ]ς\s+(εγινε|έγινε)\s+(η\s+)?(εκπαιδευ|εκπαίδευ|αξιολογ|αξιολόγ)\w*", 0.85),
    (r"\bπο[υύ]\s+(τρεχει|τρέχει|εκπαιδευ\w*)\b", 0.80),
    (r"\bfine[- ]?tun\w*", 0.75),
...
_SCHEDULE_PATTERNS: list[tuple[str, float]] = [
    ...
    # «τι θα κάνεις αύριο το απόγευμα;» — ερώτηση για το πρόγραμμα χωρίς τη
    # λέξη «πρόγραμμα». Έπεφτε στο casual, το ημερολόγιο δεν ερωτώταν, και το
    # μοντέλο επινόησε εκδρομή στο Ναύπλιο με ώρα αναχώρησης και επιστροφής.
    # Ένα φανταστικό ραντεβού είναι το χειρότερο που μπορεί να πει ένα
    # ψηφιακό δίδυμο, γιατί ακούγεται ακριβώς σαν αληθινό.
    (r"\b(τι|τί)\s+(θα\s+)?(κανεις|κάνεις|εχεις|έχεις)\b[^;.!?]{0,20}"
     r"\b(αυριο|αύριο|σημερα|σήμερα|μεθαυριο|μεθαύριο|το\s+σαββατοκυριακο)\b", 0.88),
    (r"\b(αυριο|αύριο|μεθαυριο|μεθαύριο)\s+(το\s+)?"
     r"(πρωι|πρωί|απογευμα|απόγευμα|βραδυ|βράδυ|μεσημερ\w*)\b", 0.85),
...
#: Χαιρετισμοί που αφαιρούνται πριν την ταξινόμηση.
#:
#: Μόνο στην ΑΡΧΗ του μηνύματος, και μόνο μαζί με προαιρετική προσφώνηση.
#: Ένα «καλημέρα» στη μέση πρότασης είναι περιεχόμενο· στην αρχή είναι
#: τυπικότητα. Το όνομα μετά τον χαιρετισμό («καλησπέρα γιώργο») φεύγει
#: μαζί, αλλιώς μένει σκουπίδι που δεν ταιριάζει πουθενά.
_GREETING_RE = re.compile(
    r"^\s*(?:"
    r"καλημ[εέ]ρα|καλησπ[εέ]ρα|καλην[υύ]χτα|καληνύχτα|"
    r"γει[αά]\s*(?:σου|σας|χαρά)?|χαίρετε|χαιρετε|"
    r"hi|hello|hey|good\s+(?:morning|evening|afternoon)"
    r")"
    r"(?:\s+(?:σου|σας|ρε|βρε))?"          # «γεια σας», «καλησπέρα ρε»
    r"(?:\s+[α-ωά-ώa-z]{2,15})?"           # προσφώνηση: «καλησπέρα γιώργο»
    r"\s*[,.!;:·—-]*\s*",
    re.IGNORECASE,
)
...
def classify_intent(message: str) -> ClassificationResult:
    ...
    # Χαιρετισμός: θόρυβος, όχι σήμα.
    #
    # Το «καλησπέρα» ταίριαζε casual με βάρος 0,90 — υψηλότερο από κάθε
    # μοτίβο schedule ή personal — οπότε ΚΑΘΕ μήνυμα που ξεκινά ευγενικά
    # παρακάμπτε όλη τη δρομολόγηση. Παρατηρημένες συνέπειες:
    #
    #   «καλησπέρα σας, ποιοι είναι οι περιορισμοί της εργασίας;»
    #        personal → casual   (δεν φορτώνεται η ταυτότητα)
    #   «καλημέρα, τι μου έλεγες για το σπίτι;»
    #        knowledge → casual  (δεν τρέχει ανάκτηση)
    #   «καλησπέρα γιώργο, τι θα κάνεις αύριο το απόγευμα;»
    #        casual, χωρίς ημερολόγιο → το μοντέλο επινόησε εκδρομή στο
    #        Ναύπλιο, με ώρα αναχώρησης και ώρα επιστροφής
    #
    # Είναι ο χειρότερος συνδυασμός για επίδειξη: ένας εξεταστής ρωτά
    # ευγενικά, και η ευγένεια είναι ακριβώς αυτό που στέλνει την ερώτηση
    # στη διαδρομή χωρίς τεκμήρια.
    #
    # Ταξινομείται το υπόλοιπο του μηνύματος. Αν δεν μένει τίποτα — σκέτο
    # «καλησπέρα» — ισχύει το αρχικό κείμενο και η απάντηση είναι όντως
    # casual.
    stripped = _GREETING_RE.sub("", text).strip(" ,.!;:·-—")
