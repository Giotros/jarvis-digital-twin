...
_REQUIRED_SOURCE: dict[str, tuple[str, str]] = {
    "schedule": (
        ...
        "Αν η ερώτηση αναφέρεται σε κάτι συγκεκριμένο που δεν ξέρεις "
        "(«τι ώρα θα ξεκινήσουμε;»), ΡΩΤΗΣΕ σε τι αναφέρεται. Μια ερώτηση "
        "κρατάει τη συζήτηση ανοιχτή· μια άρνηση την κλείνει.",
    ),
    "memory": (
        "email",
...
_UNGROUNDED_SCHEDULE = re.compile(
    ...
    # Ψευδής άρνηση: προσποιείται ότι ΕΓΙΝΕ ο έλεγχος.
    #
    # «Δεν είδα κάτι στο ημερολόγιο για αύριο πρωί γιατί τώρα μόλις άνοιξα
    # υπολογιστή» — ακούγεται σαν παραδοχή άγνοιας και είναι ισχυρισμός ότι
    # κοιτάχτηκε το ημερολόγιο και βρέθηκε άδειο. Πιο ύπουλο από το «δεν
    # έχω κάτι», γιατί η ίδια η μορφή της πρότασης μοιάζει με ειλικρίνεια.
    #
    # Η δικαιολογία από πάνω («μόλις άνοιξα υπολογιστή») είναι επινοημένο
    # γεγονός — το σύστημα δεν έχει υπολογιστή που ανοίγει.
    r"|δεν\s+ειδα\s+(?:κατι|τιποτα)?\s*(?:στο|στην)\s+(?:ημερολογιο|ατζεντα|calendar)"
    r"|(?:κοιταξα|ειδα|τσεκαρα)\s+(?:το\s+)?(?:ημερολογιο|ατζεντα|calendar)"
    r"|(?:μολις|τωρα)\s+(?:ανοιξα|σηκωθηκα|ξυπνησα)",
    re.IGNORECASE,
)
...
#: Η ερώτηση προϋποθέτει κοινό γεγονός που ο συνομιλητής θεωρεί γνωστό.
#:
#: «Αύριο το πρωί τι ώρα θα **ξεκινήσουμε**;» δεν ρωτά τι έχει το
#: ημερολόγιο — ρωτά για κάτι συγκεκριμένο που ο άλλος θεωρεί δεδομένο.
#: Η άρνηση «δεν βλέπω το ημερολόγιό μου» είναι αληθής και άχρηστη: ο
#: συνομιλητής δεν ρώτησε αυτό, και η συζήτηση σταματά.
#:
#: Αυτό που κάνει ένας άνθρωπος όταν δεν θυμάται είναι να **ρωτήσει**:
#: «για τι πράγμα λες;». Δηλώνει την ίδια άγνοια και κρατά τη σειρά ανοιχτή
#: — και σε επίδειξη είναι η διαφορά ανάμεσα σε αδιέξοδο και διάλογο.
#:
#: Σημάδια: πρώτο πληθυντικό (κοινό σχέδιο), οριστικό άρθρο σε ουσιαστικό
#: γεγονότος, δεικτική αναφορά.
_PRESUPPOSES_EVENT = re.compile(
    r"\b(?:θα|να)\s+\w+(?:ουμε|ούμε|ομαστε|όμαστε)\b"      # «θα ξεκινήσουμε»
    r"|\bτι\s+ωρα\s+(?:θα|ειναι)\b"                         # «τι ώρα θα…»
    r"|\b(?:η|την|στη|στην)\s+(?:συναντηση|εκδηλωση|παρουσιαση|"
    r"δοκιμη|προβα|εξεταση|τελετη)\b"
    r"|\b(?:το|στο)\s+(?:ραντεβου|μαθημα|meeting|call)\b"
    r"|\bαυτο\s+που\s+(?:ειπαμε|λεγαμε|κανονισαμε)\b",
    re.IGNORECASE,
)
...
#: Όταν η ερώτηση προϋποθέτει γεγονός: ζήτα το γεγονός, μην απαντήσεις.
_SCHEDULE_ASK_WHICH = (
    "Σε τι αναφέρεσαι; Δεν μπορώ να δω το ημερολόγιό μου αυτή τη στιγμή, "
    "οπότε πες μου για ποιο πράγμα λες και τα κανονίζουμε."
)


def _schedule_reply_for(message: str) -> str:
    """Άρνηση προσαρμοσμένη στο τι ρωτήθηκε."""
    if _PRESUPPOSES_EVENT.search(_fold_accents(message)):
        return _SCHEDULE_ASK_WHICH
    return _SCHEDULE_REFUSAL


def _asserts_ungrounded_schedule(reply: str) -> bool:
    """True μόνο για ΙΣΧΥΡΙΣΜΟΥΣ, όχι για προτάσεις.

    «Να έρθω κατά τις 11;» και «θα πάω στο πάρτυ» έχουν το ίδιο ρήμα και
    εντελώς διαφορετική επιστημολογία. Το πρώτο προτείνει και ρωτά — είναι
    ακριβώς αυτό που κάνει ένας άνθρωπος που δεν ξέρει την ώρα, και ήταν η
    καλύτερη απάντηση που έδωσε το σύστημα σε όλη τη μέρα. Το δεύτερο
    δηλώνει γεγονός που δεν έχει πηγή.

    Η διάκριση γίνεται ανά πρόταση: οι ερωτηματικές εξαιρούνται. Ένα φίλτρο
    που έσβηνε και τα δύο θα τιμωρούσε τη σωστή συμπεριφορά — το ίδιο
    σφάλμα με τη μετρική κάλυψης που βαθμολογούσε «χωρίς περιεχόμενο» τον
    τίτλο της εργασίας.
    """
    ...
    for sentence in re.split(r"(?<=[.!;?·])\s+|\n+", folded):
        ...
        if not stripped or stripped.endswith(("?", ";")):
            continue
        if _UNGROUNDED_SCHEDULE.search(stripped):
            return True
    return False


def _refuse_ungrounded_schedule(
    reply: str, gathered: "ContextResponse", message: str = ""
) -> tuple[str, bool]:
    ...
    if calendar_ok or not _asserts_ungrounded_schedule(reply):
        return reply, False
    logger.info("Αντικαταστάθηκε ισχυρισμός για ασύνδετο ημερολόγιο")
    return _schedule_reply_for(message), True
